...
import { API_URL_PRODUCT_ID, BASE_URL, API_URL_PRODUCT_LIST, API_URL_PRODUCT_CATEGORY } from '../../constant/config'
...
export const actGetProductsByCategories = (pageIndex = 0, pageSize = 5, id) => {
    return (dispatch) => {
        callApi(`${API_URL_PRODUCT_CATEGORY}?pageSize=${pageSize}&pazeIndex=${pageIndex}&categoryId=${id}`, 'GET', null).then(res => {
            dispatch(actGetProductsByCategoriesRequest(res.data, res.total))
        })
    }
}


export const actGetProductsByCategoriesRequest = (dulieu) => {
    return {
        type: Types.GET_PRODUCT_CATEGORY,
        dataOfProductCA: dulieu
    }
}
    ...
    if (action !== undefined) {
        switch (action.type) {
            ...
            case Types.GET_PRODUCT_CATEGORY:
                return {...state, dataProductCA: action.dataOfProductCA }
            default:
                return state
